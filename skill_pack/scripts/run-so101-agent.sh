#!/usr/bin/env bash
# ============================================================================
#  octos LLM agent drives the SO-101 pick-and-place from a sentence.
#
...
#  reBot-specific grasp geometry lives in manifests/so101.json (ROBOT_MANIFEST);
...
LOG=/tmp/so101-agent.log
HEADLESS="${HEADLESS:-0}"
SENTENCE="${*:-Pick up the red cube and place it on the green plate}"
DRIVER="${DRIVER:-agent}"

die() { echo "[so101-agent] ERROR: $*" >&2; exit 1; }

echo "[so101-agent] preflight checks…"
...
PYCHK
  echo "[so101-agent] preflight OK (model, manifest, dora, Ollama+$OLLAMA_MODEL, octos_py)"
else
  echo "[so101-agent] preflight OK (model, manifest, dora; DRIVER=skill — no LLM needed)"
fi
...
# keeps up. Override with EXEC_TICK_MS.
...
echo "[so101-agent] resetting dora daemon + killing node orphans + freeing ports…"
...
echo "[so101-agent] starting dataflow (log: $LOG)…"
dora start "$YML" --attach > "$LOG" 2>&1 &
DORA_PID=$!

echo "[so101-agent] waiting for bridge + object server (up to 5 min)…"
bridge_ok=0
for _ in $(seq 1 300); do
  curl -fsS -m2 "$URL/healthz" >/dev/null 2>&1 && { bridge_ok=1; break; }
  kill -0 "$DORA_PID" 2>/dev/null || { echo "[so101-agent] dataflow exited early:"; tail -30 "$LOG"; exit 1; }
  sleep 1
done
...
echo
if [ "$DRIVER" = "agent" ]; then
  echo "[so101-agent] >>> handing this sentence to the octos agent ($OLLAMA_MODEL):"
  echo "[so101-agent] >>> \"$SENTENCE\""
  CMD=("$PY" "$SKILL/arm_agent.py" "$SENTENCE")
else
  echo "[so101-agent] >>> running deterministic skill-level pick-and-place (no LLM)"
...
if [ "$HEADLESS" = "1" ]; then
  echo "[so101-agent] headless run done."
else
  echo "[so101-agent] done. Viewer + dataflow stay up — Ctrl-C to tear down."
  wait "$DORA_PID"
fi
